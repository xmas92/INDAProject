...
public class CopyTextField extends AbstractUserInterface {
	...
	public CopyTextField(SpriteSheet ss) {
	...
	public void Draw() {
		...
		if (rec.height - 2 * margin >= f.getLineHeight()) {
			if(w > graphic.getWidth() - 3*margin) {
				...
				ChatboxTest.agc.getGraphics().drawString(tempString, rec.x + 2*margin, rec.y + margin);
			} else {
				ChatboxTest.agc.getGraphics().drawString(s, rec.x + 2*margin, rec.y + margin);	
			}
		}
	}
